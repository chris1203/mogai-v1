class JobsController < ApplicationController
  ...
  def index
    @jobs = Job.all
    if params[:search]
      @jobs = Job.published.search(params[:search]).recent
    elsif
  ...
  def job_params
  params.require(:job).permit(:title, :city, :description, :wage_upper_bound, :wage_lower_bound, :contact_email,:is_hidden)
  end
end
